...
class MedicineViewSet(viewsets.ViewSet):
    ...
    def list(self,request):
        medicine=Medicine.objects.raw('SELECT * FROM DjangoMedicalApp_medicine')
...
class BillDetailsViewset(viewsets.ViewSet):
    ...
    def create(self,request):
        try:
            ...
            serializer.is_valid(raise_exception=True)
            serializer.save()
            dict_response={"error":False,"message":"Bill Details Data Save Successfully"}
        except:
            dict_response={"error":True,"message":"Error During Saving Company Account Data"}
        return Response(dict_response)

    def list(self,request):
        billdetails=BillDetails.objects.all()
        serializer=BillDetailsSerializer(billdetails,many=True,context={"request":request})
    ...
    def retrieve(self,request,pk=None):
        ...
        serializer=BillDetailsSerializer(billdetails,context={"request":request})
        return Response({"error":False,"message":"Single Data Fetch","data":serializer.data})

    def update(self,request,pk=None):
        queryset=BillDetails.objects.all()
        billdetails=get_object_or_404(queryset,pk=pk)
        serializer=BillDetailsSerializer(billdetails,data=request.data,context={"request":request})
        serializer.is_valid()
        serializer.save()
        return Response({"error":False,"message":"Data Has Been Updated"})
...
class GenerateBillViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        for medicine_detail in request.data["medicine_details"]:
            ...
            medicine_detail1["discount_amount"]= medicine_detail["discount_amount"]
...
class HomeApiViewset(viewsets.ViewSet):
    ...
    def list(self,request):
        ...
        for bill in bill_details:
            ...
            sell_amt=float((sell_amt+float(bill.medicine_id.sell_price)*int(bill.qty))-float(bill.discount_amount))
            # print(f"sell_amt={sell_amt}, buy_amt={buy_amt}")
        ...
        # current_year_365days=current_date1+timedelta(days=365)
        # current_year_365days=current_year_365days.strftime('%Y-%m-%d')

        bill_details_today=BillDetails.objects.filter(added_on__date=current_date)

        # bill_details_year=BillDetails.objects.filter(added_on__date=current_year_365days)
        # bill_details_week = BillDetails.objects.filter(added_on__date=current_date_7days)
        ...
        for bill in bill_details_today:
            ...
            sell_amt_today=float(sell_amt_today+(float(bill.medicine_id.sell_price)*int(bill.qty))-float(bill.discount_amount))
            profit_amt_today=sell_amt_today-buy_amt_today
            # print(f"sell_amt_today={sell_amt_today}, buy_amt_today={buy_amt_today}")


        #for year
        # for bill in bill_details_year:
        #     buy_amt_year=float(buy_amt_year+float(bill.medicine_id.buy_price)*int(bill.qty))
        #     sell_amt_year=float(sell_amt_year+(float(bill.medicine_id.sell_price)*int(bill.qty))-float(bill.discount_amount))
        #     profit_amt_year=sell_amt_year-buy_amt_year
        #     print(f"sell_amt_year={sell_amt_year}, buy_amt_year={buy_amt_year}")


        #for Week
        # for bill in bill_details_week:
        #     buy_amt_week=float(buy_amt_week+float(bill.medicine_id.buy_price)*int(bill.qty))
        #     sell_amt_week=float(sell_amt_week+(float(bill.medicine_id.sell_price)*int(bill.qty))-float(bill.discount_amount))
            # print('for week'f"sell_amt_year={sell_amt_week}")
            # profit_amt_week=sell_amt_week-buy_amt_week
        ...
        for billdate in bill_dates:
            ...
            for billsingle in bill_data:
                ...
                sell_amt_inner = float((sell_amt_inner + float(billsingle.medicine_id.sell_price) * int(billsingle.qty))-float(bill.discount_amount))
        ...
        dict_respone={"error":False,"message":"Home Page Data","customer_request":len(customer_request_serializer.data),"bill_count":len(bill_count_serializer.data),"medicine_count":len(medicine_count_serializer.data),"company_count":len(company_count_serializer.data),"employee_count":len(employee_count_serializer.data),"sell_total":sell_amt,"buy_total":buy_amt,"profit_total":profit_amt,"discount_amount":discount_amount,"request_pending":len(customer_request_pending_serializer.data),"request_completed":len(customer_request_completed_serializer.data),"profit_amt_today":profit_amt_today,"sell_amt_today":sell_amt_today, "medicine_expire_serializer_data":len(medicine_expire_serializer.data),"sell_chart":sell_chart_list,"buy_chart":buy_chart_list,"profit_chart":profit_chart_list}
